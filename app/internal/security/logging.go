...
func (l *Logger) SecurityEvent(eventType SecurityEventType, actorID *int, actorEmail, ipAddress, userAgent string, extra map[string]interface{}) {
	...
	// Log to database (audit_log table) - SR-010
	objectType := ""
	objectID := ""
	if extra != nil {
		if ot, ok := extra["object_type"].(string); ok {
			objectType = ot
		}
		if oid, ok := extra["object_id"].(string); ok {
			objectID = oid
		}
	}

	if err := l.logToDatabase(eventType, actorID, objectType, objectID, ipAddress, userAgent); err != nil {
		l.Warn(fmt.Sprintf("Failed to write audit log to database: %v", err))
	}
}

// logToDatabase writes security events to audit_log table
func (l *Logger) logToDatabase(eventType SecurityEventType, actorID *int, objectType, objectID, ipAddress, userAgent string) error {
	if database.DB == nil {
		return fmt.Errorf("database not connected")
	}

	query := `
		INSERT INTO audit_log (actor_id, action, object_type, object_id, ip_address, user_agent)
		VALUES ($1, $2, $3, $4, $5, $6)
	`
